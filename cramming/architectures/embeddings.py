...
class PositionalEmbedding(torch.nn.Module):
    ...
    def __init__(self, demb):
        ...
        inv_freq = (
            1 / (10000 ** (torch.arange(0.0, demb, 2.0) / demb))
        ).float()
...
class RandomNoise(torch.nn.Module):
    ...
    def forward(self, input_ids):
        return torch.normal(
            0,
            0.1,
            size=(input_ids.size(0), input_ids.size(1), self.embedding_dim),
        ).to(input_ids.device)
...
class LearnablePositionalRand(torch.nn.Module):
    ...
    def forward(self, input_ids):
        ...
        position_ids = torch.sort(
            torch.randperm(max_length, dtype=torch.long, device=device)[
                :seq_length
            ]
        ).values
        return self.embedding(position_ids)
...
class Rotary(torch.nn.Module):
    def __init__(self, dim, base=10000, def_seq_length=128, seq_dim: int = 0):
        ...
        def rotate_half(x: torch.Tensor):
            x1, x2 = (
                x[..., : x.shape[-1] // 2],
                x[..., x.shape[-1] // 2 :],
            )  # torch.split(x, x.shape[-1] // 2, dim=-1)  # not faster
        ...
        def rope_fn(
            ...
            rotated = (
                QK * cos[: QK.shape[0]] + rotate_half(QK) * sin[: QK.shape[0]]
            )
    ...
    def _get_cos_sin(self):
        ...
        if self.seq_dim == 0:
            return (
                emb.cos()[:, None, None, :].detach(),
                emb.sin()[:, None, None, :].detach(),
            )
        else:
            return (
                emb.cos()[None, :, None, :].detach(),
    ...
    def single_forward(self, inputs: torch.Tensor):
        ...
        cos, sin = (
            self.cos_cached[: inputs.shape[0]],
            self.sin_cached[: inputs.shape[0]],
        )
...
class RotarySanityCheck(torch.nn.Module):
    ...
    def forward(self, query_layer: torch.Tensor, key_layer: torch.Tensor):
        ...
        return (query_layer * cos) + (self.rotate_half(query_layer) * sin), (
            key_layer * cos
        ) + (self.rotate_half(key_layer) * sin)
...
class RotaryEleutherAI(torch.nn.Module):
    ...
    def __init__(self, dim_model: int, *_, **__):
        ...
        inv_freq = 1.0 / (
            10000 ** (torch.arange(0, dim_model, 2).float() / dim_model)
        )
        self.register_buffer("inv_freq", inv_freq)

        _cos_cached, _sin_cached = self._update_cos_sin_tables(
            torch.randn(1, 128, 1), seq_dimension=-2
        )
    ...
    def forward(
        ...
        return (
            self.apply_rotary_pos_emb(
                q, self._cos_cached, self._sin_cached, seq_dimension
            ),
            self.apply_rotary_pos_emb(
                k, self._cos_cached, self._sin_cached, seq_dimension
            ),
        )
...
class RotaryLLAMA(torch.nn.Module):
    ...
    def __init__(
        self, hidden_per_head, base=10000, max_seq_length=512, seq_dim: int = 0
    ):
        super().__init__()
        self.seq_dim: int = seq_dim
        freqs_cis = self.precompute_freqs_cis(
            dim=hidden_per_head, end=max_seq_length * 2, theta=base
        )
        self.register_buffer("freqs_cis", freqs_cis)

    def forward(self, query_layer: torch.Tensor, key_layer: torch.Tensor):
        return self.apply_rotary_emb(
            query_layer, key_layer, freqs_cis=self.freqs_cis
        )
    ...
    def reshape_for_broadcast(self, freqs_cis: torch.Tensor, x: torch.Tensor):
        ...
        shape = [
            s if i == self.seq_dim or i == x.ndim - 1 else 1
...
class FIRE(torch.nn.Module):
    def __init__(
        ...
        self.init_L = torch.nn.Parameter(
            torch.tensor(init_L), requires_grad=False
        )
    ...
    def forward(self, seq_length, device):
        ...
        positions = torch.sort(
            torch.randperm(max_length, dtype=torch.float, device=device)[
                :seq_length
            ]
        ).values
        ...
        relative_distances = torch.log(
            torch.abs(self.c * relative_distances) + 1
        )
        position_normalizer = torch.log(
            torch.abs(self.c * position_normalizer) + 1
        )

        # Progressive interpolation
        normalized_distances = relative_distances / (
            position_normalizer + self.eps
        )
...
class Abacus(torch.nn.Module):
    ...
    def helper(self, delimiters_mask, spaces_mask):
        B, T = delimiters_mask.shape
        device = delimiters_mask.device

        index = torch.arange(T).repeat(B, 1).to(device)
        zeros = torch.zeros_like(delimiters_mask).long()

        segment_ids = torch.cumsum(delimiters_mask, dim=1)
        segment_idx_step_fn = zeros.scatter_add(
            1, segment_ids, (index + 1) * delimiters_mask.long()
        ).gather(1, segment_ids)

        spaces_cumsum = torch.cumsum(spaces_mask, dim=1)
        space_step_fn_per_segment = spaces_cumsum - zeros.scatter_add(
            1, segment_ids, spaces_cumsum * delimiters_mask.long()
        ).gather(1, segment_ids)
    ...
    def forward(self, input_ids):
        ...
        if self.training:
            ...
            output[
                output > 0
            ] += k  # as we already have ones in the tensor, the tensor values will be k+1
